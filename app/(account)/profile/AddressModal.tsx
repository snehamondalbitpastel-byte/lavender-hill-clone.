...
export default function AddressModal({
  ...
  async function save(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setError(d.error || "Couldn't save the address.");
      setBusy(false);
    }
  }
      ...
      <div className="relative z-[1] flex max-h-[90vh] w-full max-w-[560px] flex-col overflow-hidden rounded-2xl bg-white shadow-xl">
        ...
        <form onSubmit={save} className="flex flex-col gap-3.5 overflow-y-auto px-6 pb-6">
          ...
          <div className="grid grid-cols-2 gap-3.5">
            <Field id="addr-fn" label="First name" value={firstName} onChange={setFirstName} />
            <Field id="addr-ln" label="Last name" value={lastName} onChange={setLastName} />
          </div>
          <Field id="addr-co" label="Company" value={company} onChange={setCompany} />
          <Field id="addr-a1" label="Address" value={address1} onChange={setAddress1} />
          <Field id="addr-a2" label="Apartment, suite, etc" optional value={address2} onChange={setAddress2} />
          <div className="grid grid-cols-3 gap-3.5">
            <Field id="addr-city" label="City" value={city} onChange={setCity} />
            {country === "India" ? (
              <div className="relative">
                <select
                  id="addr-state"
              ...
              <Field id="addr-state" label="State" value={state} onChange={setState} />
            )}
            <Field id="addr-pc" label="Postcode" value={postcode} onChange={setPostcode} />
          </div>

          {/* Phone */}
          <div>
            <label className="mb-1 block text-[0.72rem] text-[var(--acc-muted)]">Phone</label>
          ...
          <label className="mt-1 flex cursor-pointer select-none items-center gap-2.5">
            ...
            <span className="text-[0.9rem] text-[var(--acc-fg)]">This is my default address</span>
          </label>
